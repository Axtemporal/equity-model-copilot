...
- **Pending:** telecom engine one step behind O&G (schedules still frozen) + its divergent financial schema; generalizing the financial input template across sectors; full three-scenario premise sets in the model (not just valuation-level); per-asset vs. consolidated structure detection refinements.
